...
FIRST_PORT, LAST_PORT = 1, 65535

print(f"=== BẮT ĐẦU QUÉT CỔNG {FIRST_PORT}-{LAST_PORT} TRÊN {target_ip} ===")
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
    ...
    executor.map(scan_port, range(FIRST_PORT, LAST_PORT + 1))

# Đóng dấu thời gian kết thúc
end_time = time.time()
open_ports.sort()   # sắp xếp cho dễ đọc (các luồng ghi vào theo thứ tự ngẫu nhiên)
...
print(f"- Thời gian hoàn thành quét {LAST_PORT - FIRST_PORT + 1} cổng: {round(end_time - start_time, 2)} giây")
print("="*50)
